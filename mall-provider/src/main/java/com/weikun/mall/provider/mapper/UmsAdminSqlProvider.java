...
import com.weikun.api.model.UmsAdminExample;
...
public class UmsAdminSqlProvider {
    ...
    protected void applyWhere(SQL sql, UmsAdminExample example, boolean includeExamplePhrase) {
        ...
        List<UmsAdminExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            UmsAdminExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<UmsAdminExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    UmsAdminExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {

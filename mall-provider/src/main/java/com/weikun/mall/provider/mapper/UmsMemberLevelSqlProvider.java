...
public class UmsMemberLevelSqlProvider {
    ...
    protected void applyWhere(SQL sql, UmsMemberLevelExample example, boolean includeExamplePhrase) {
        ...
        List<UmsMemberLevelExample.Criteria> oredCriteria = example.getOredCriteria();
        boolean firstCriteria = true;
        for (int i = 0; i < oredCriteria.size(); i++) {
            UmsMemberLevelExample.Criteria criteria = oredCriteria.get(i);
            if (criteria.isValid()) {
                if (firstCriteria) {
                    firstCriteria = false;
                } else {
                ...
                sb.append('(');
                List<UmsMemberLevelExample.Criterion> criterions = criteria.getAllCriteria();
                boolean firstCriterion = true;
                for (int j = 0; j < criterions.size(); j++) {
                    UmsMemberLevelExample.Criterion criterion = criterions.get(j);
                    if (firstCriterion) {
